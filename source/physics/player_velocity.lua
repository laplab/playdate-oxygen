...
function update_player_velocity(player, dt)
    ...
    elseif not player.attached_left and playdate.buttonIsPressed(playdate.kButtonLeft) then
        player.velocity.x = -PLAYER_MAX_H_SPEED
        player.flip = true
    elseif not player.attached_right and playdate.buttonIsPressed(playdate.kButtonRight) then
        player.velocity.x = PLAYER_MAX_H_SPEED
        player.flip = false
    end
    ...
    elseif (player.attached_left or player.attached_right) and playdate.buttonJustPressed(playdate.kButtonA) then
        ...
        player.velocity.y -= JUMP_SPEED
        player.velocity.x = 3 * attach_sign * PLAYER_MAX_H_SPEED
    elseif not player.grounded then
        if player.attached_left or player.attached_right then
            player.velocity.y = 0 -- ATTACHED_SPEED
        else
